...
seed 15の29 kill runは最終ammo 0、health -4だった。168回FIREを判断した一方、
実際のammo decrementは52回。停止世界では照準と反応速度が十分すぎるほど改善し、
最後はAIMではなく弾薬枯渇が生存上限になった。「Cloud LLMが遅くて撃てない」から
「撃ちすぎて弾がなくなる」へ、ボトルネックが丸ごと移動した回である。
